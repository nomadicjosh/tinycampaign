...
/**
 * tinyCampaign Bounce Mail Handler
 *
 * @license GPLv3
 *         
 * @since 2.0.0
 ...
 */
function bounce_body_rules($body,$structure,$debug_mode=false) {
 ...
 *                      if we could NOT detect the type of bounce, return rule_no = '0000'
 */
function bounce_dsn_rules($dsn_msg,$dsn_report,$debug_mode=false) {
  // initialize the result array
  $result = array(
    'email'        => ''
    ,'bounce_type' => false
  ...
  if ($result['rule_no'] == '0000') {
    if ($debug_mode) {
      echo _t('email: ') . $result['email'] . $bmh_newline;
      echo _t('Action: ') . $action . $bmh_newline;
      echo _t('Status: ') . $status_code . $bmh_newline;
      echo _t('Diagnostic-Code: ') . $diag_code . $bmh_newline;
      echo _t('DSN Message:') . "<br />\n" . $dsn_msg . $bmh_newline;
      echo $bmh_newline;
    }
  } else {
    if ($result['bounce_type'] === false) {
 ...
 * @return boolean
 */
function bounce_callback_action ($msgnum, $bounce_type, $email, $subject, $xheader, $remove, $rule_no=false, $rule_cat=false, $totalFetched=0) {

  // sample mysql code
  if ( $remove == true || $remove == '1' ) {
    echo _t("note: code would have set the database to allowed = 'false'");
    /*
  ...
  $displayData = bounce_prep_data($email, $bounce_type, $remove);
...
function bounce_prep_data($email, $bounce_type, $remove) {
