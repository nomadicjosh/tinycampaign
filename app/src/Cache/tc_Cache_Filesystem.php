...
/**
 * tinyCampaign Filesystem Cache Class.
 *
 * @license GPLv3
 *         
 * @since 2.0.0
 ...
 */
class tc_Cache_Filesystem extends \app\src\Cache\tc_Abstract_Cache
{
    ...
    public function __construct(\Liten\Liten $liten = null)
    {
         ...
         */
        if (!is_dir($cacheDir) || !tc_file_exists($cacheDir, false)) {
            try {
                _mkdir($cacheDir);
            } catch (IOException $e) {
    ...
    protected function removeStaleCache($namespace = 'default', $ttl = 0)
    {
        ...
        if (is_array($stale)) {
            foreach ($stale as $filename) {
                if (tc_file_exists($filename, false)) {
